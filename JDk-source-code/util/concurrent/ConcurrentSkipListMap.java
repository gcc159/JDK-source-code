...
public class ConcurrentSkipListMap<K,V> extends AbstractMap<K,V>
    ...
    static final class Node<K,V> {
         ...
         */
        //加上一个标志该节点将要删除的Marker.一个节点被作为marker的标志是
        //该节点的value指向其本身.
        ...
        void helpDelete(Node<K,V> b, Node<K,V> f) {
            ...
            if (f == next && this == b.next) {
                //f为当前节点的后继,b为前驱,该方法为了保证当前该节点仍然位于此两个节点的中间
                if (f == null || f.value != f) // not already marked
                    //如果当前节点的后继为一个待删除节点,准备使用casNext操作将其删除(断开链接,使其可以被回收)
     ...
     */
    //索引节点,当前包含一个当前节点,一个下部索引/数据节点,一个指向下一个的索引节点.
        ...
        for (;;) {
            for (Index<K,V> q = head, r = q.right, d;;) {
                if (r != null) {
                    ...
                    K k = n.key;
                    if (n.value == null) {
                        //发现值为null,表名该节点处于待删除状态,则尝试unlink
                        if (!q.unlink(r))
                            break;           // restart  删除失败,则重新开始循环
                        r = q.right;         // reread r 因为后一个节点已经被删除,所以重新开始再读下一个有效的节点
                        continue;
                    }
                    if (cpr(cmp, key, k) > 0) {
                        //当前key比k的值要大,则继续向右查找.
                ...
                //到达此处有两种情况,如果r为null或者在上面的cpr比较小于,则说明q为前驱节点
                if ((d = q.down) == null)
                    //下一个为null,表示已经搜索到叶链表,则可以之前返回其前驱节点.
                    return q.node;
                //找到了当前层的前驱节点,向下继续查找
                q = d;
                r = d.right;
     ...
     */
    //找到包含key的节点,如果没找到则直接删除那些value==null的节点
    ...
    private V doPut(K key, V value, boolean onlyIfAbsent) {
        ...
        outer: for (;;) {
            ...
            //n为当前节点的后继节点
            for (Node<K,V> b = findPredecessor(key, cmp), n = b.next;;) {
                //n为null,表示当前只需要插入到链表尾部即可.否则需要进行操作
                if (n != null) {
                    Object v; int c;
                    Node<K,V> f = n.next;
                    if (n != b.next)               // inconsistent read
                        ...
                        break;
                    if ((v = n.value) == null) {   // n is deleted
                        //如果下一个节点的值为null,表示这个节点已经被删除.
                        //调用HelpDelete删除后面那个节点.
                        n.helpDelete(b, f);
                        break;
                    }
                    //前驱被删除,需要重新开始加入
                    ...
                    //key相等,把后面的值设为value,put操作变成update操作
                    if (c == 0) {
        ...
        if ((rnd & 0x80000001) == 0) { // test highest and lowest bits
            ...
            if (level <= (max = h.level)) {
                ...
                //概率测出来的level小于当前max-level,则只增加一层level
        ...
        outer: for (;;) {
            for (Node<K,V> b = findPredecessor(key, cmp), n = b.next;;) {
                ...
                if (value != null && !value.equals(v))
                    //出现了数据不一致现象
                    break outer;
                //将n的value标识为null,标识其已经被删除
                if (!n.casValue(v, null))
                    break;
                //给当前需要删除的节点加上一个Marker
                if (!n.appendMarker(f) || !b.casNext(n, f))
                    //更新失败,将根据上面已经表示为null,剔除所有value为null的节点
                    findNode(key);                  // retry via findNode
                else {
                    //进入此处表示添加marker和原子将节点从链表中剔除都已经成功
                    //既然该节点已经被表值删除成功,则清楚所有指向该叶子节点的index
                    findPredecessor(key, cmp);      // clean index
                    //第一层只有一个头索引,需要将其索引的数量缩小一层.
                    if (head.right == null)
                        tryReduceLevel();
                }
                @SuppressWarnings("unchecked") V vv = (V)v;
